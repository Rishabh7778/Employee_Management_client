...
const List = () => {
  ...
  const fetchLeaves = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/api/leave/${id}`, {
        headers: {
            ...
            {leaves.length > 0 ? (
              leaves.map((leave) => (
                <tr key={leave._id} className="hover:bg-gray-100 transition duration-200">
                  ...
                  <td className="py-3 px-4 border text-center">{new Date(leave.startDate).toLocaleDateString()}</td>
                  <td className="py-3 px-4 border text-center">{new Date(leave.endDate).toLocaleDateString()}</td>
                  <td className="py-3 px-4 border text-center">{leave.reason}</td>
                  <td
                    className={`py-3 px-4 border text-center font-semibold ${leave.status === "Approved" ? "text-green-600" : "text-red-600"
                      }`}
                  >
                    {leave.status}
                  </td>
